...
async function loadStats() {
    try {
        ...
        if (normEl) normEl.textContent = `✅ В норме: ${stats.normal_count} из ${stats.total_count}`;
...
function formatDate(timestamp, minutes) {
    ...
    if (m <= 60) return date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit', second: '2-digit' });
    if (m <= 720) return date.toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' });
...
async function loadCharts(minutes) {
    ...
    try {
        const res = await fetch(`index.php?action=history&minutes=${minutes}&boiler=${activeBoiler}`);
        ...
        let maxTicks = minutes <= 60 ? 12 : (minutes <= 360 ? 10 : (minutes <= 1440 ? 8 : 6));
        
        const datasets = [
            { param: 'flue_gas_temp', label: 'Температура уходящих газов, °C', color: '#ef4444', chartId: 'chartFlueGas' },
            { param: 'steam_pressure', label: 'Давление пара, кгс/см²', color: '#3b82f6', chartId: 'chartSteamPressure' },
            { param: 'gas_flow', label: 'Расход газа, тыс. м³/ч', color: '#22c55e', chartId: 'chartGasFlow' },
        ];
        ...
        renderChart('chartEfficiency', 'КПД, %', labels, efficiency, '#f59e0b', maxTicks);
...
function renderChart(canvasId, label, labels, data, color, maxTicksLimit = 10) {
    ...
    let displayLabels = labels, displayData = data;
    ...
    chartInstances[canvasId] = new Chart(ctx, {
        ...
        options: {
            ...
            scales: {
                x: {
                    ticks: { color: '#94a3b8', maxTicksLimit: maxTicksLimit, maxRotation: labels.length > 50 ? 45 : 0 },
...
async function load() {
    try {
        ...
        const load = parseFloat(json.fact.load);
        const nominal = parseFloat(json.boiler_nominal) || 480;
        const loadPercent = load / nominal * 100;
        
        const loadRow = document.createElement('tr');
        if (loadPercent >= 60 && loadPercent <= 80) loadRow.className = 'load-normal';
        else if ((loadPercent >= 40 && loadPercent < 60) || (loadPercent > 80 && loadPercent <= 100)) loadRow.className = 'load-warning';
        else loadRow.className = 'load-danger';
        loadRow.innerHTML = `<td>Нагрузка</td><td>${load} т/ч</td><td>${load} т/ч</td><td>0</td><td>✓</td>`;
...
async function sendRequest(url, options, successMessage, errorMessage) {
    try {
        ...
        if (data.success) { if (successMessage) showMessage('success', successMessage); }
        else { if (errorMessage) showMessage('error', errorMessage || data.message); }
        return data;
    } catch (e) {
...
async function exportToExcel() {
    ...
    try {
        const res = await fetch(`index.php?action=history&minutes=${minutes}&boiler=${activeBoiler}`);
        const data = await res.json();
        if (!data.length) { showMessage('error', 'Нет данных'); return; }
        ...
        let html = '<table border="1"><tr><th>Время</th><th>Нагрузка</th>';
        labels.forEach(l => html += `<th>${l}</th>`);
        html += '</tr>';
        data.forEach(row => {
            html += `<tr><td>${row.timestamp}</td><td>${row.load}</td>`;
            params.forEach(p => html += `<td style="mso-number-format:'\\@';">${row[p] ?? '-'}</td>`);
            html += '</tr>';
        });
        html += '</table>';
...
let soundEnabled = false, alertSound = null, audioUnlocked = false;
function loadSoundState() { soundEnabled = localStorage.getItem('soundEnabled') === 'true'; updateSoundButton(); }
function saveSoundState() { localStorage.setItem('soundEnabled', soundEnabled); }
function initAudio() { if (!alertSound) { alertSound = new Audio('sounds/warn.mp3'); alertSound.volume = 0.6; } }
function unlockAudio() {
    if (audioUnlocked || !alertSound) return;
    alertSound.play().then(() => { alertSound.pause(); alertSound.currentTime = 0; audioUnlocked = true; updateSoundButton(); if (soundEnabled) playSound(); }).catch(() => {});
}
function playSound() { if (soundEnabled && audioUnlocked && alertSound) { alertSound.currentTime = 0; alertSound.play().catch(() => {}); } }
function updateSoundButton() {
    const btn = document.getElementById('soundToggle');
    if (!btn) return;
    btn.textContent = soundEnabled && audioUnlocked ? '🔊 Звук включён' : '🔇 Звук выключен';
    btn.style.background = soundEnabled && audioUnlocked ? '#22c55e' : '#ef4444';
}
function toggleSound() {
    if (!audioUnlocked) { unlockAudio(); soundEnabled = true; saveSoundState(); updateSoundButton(); }
    else { soundEnabled = !soundEnabled; saveSoundState(); updateSoundButton(); if (soundEnabled) playSound(); }
}
document.addEventListener('DOMContentLoaded', () => {
    initAudio(); loadSoundState();
    document.getElementById('soundToggle')?.addEventListener('click', toggleSound);
});

let alertElement = null, lastAlertTime = 0;
...
function playAlert() {
    ...
    if (alertElement) { alertElement.remove(); alertElement = null; }
    ...
    const blinkInterval = setInterval(() => {
        if (!alertElement) { clearInterval(blinkInterval); return; }
        blink++;
        alertElement.style.backgroundColor = blink % 2 === 0 ? 'rgba(220,38,38,0.2)' : 'rgba(220,38,38,0.5)';
    ...
    setTimeout(() => {
        if (alertElement) { clearInterval(blinkInterval); alertElement.remove(); alertElement = null; }
    }, 10000);
}
...
async function loadBoilers() {
    try {
        ...
        if (select) {
            ...
            select.value = saved && boilers.find(b => b.code === saved) ? saved : boilers[0]?.code || '';
        ...
        console.error('Ошибка загрузки котлов:', e);
    }
}
...
document.getElementById('boilerSelect').addEventListener('change', function() {
    ...
    load();
    loadStats();
    showMessage('success', 'Котел выбран')
});
            ...
            filtered.forEach(ref => {
                tbody.innerHTML += `
                    <tr>
                        ...
                        <td><input type="number" step="0.001" value="${ref.reference_value}" data-param="${ref.parameter_id}"></td>
                        <td><input type="number" step="0.001" value="${ref.max_deviation}" data-param="${ref.parameter_id}"></td>
                    </tr>
                `;
            });
        }
